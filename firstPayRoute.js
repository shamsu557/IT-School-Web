        ...
        if (result.length === 1) {
            ...
            db.query(updateQuery, [firstPayreferenceNumber, existingAdmissionNumber, existingAdmissionNumber, existingAdmissionNumber], (updateErr, updateResult) => {
                ...
                if (updateResult.affectedRows === 1) {
                    const paymentMessage = `Dear ${firstName} , Your payment for the first installment fee of ${firstInstallmentFee} with reference number ${firstPayreferenceNumber} has been verified successfully! Use your admission number ${existingAdmissionNumber} as username and password to login.`;
                    res.send(`
                        <!DOCTYPE html>
                        <html lang="en">
                        <head>
